# Sticky-header в редакторе: технический обзор текущей реализации

## О чем этот документ

Этот документ описывает текущее устройство sticky-header в редакторе. Он фиксирует состав механизма, поведение во время работы, текущие контракты, инварианты, константы и ограничения реализации.

Документ ограничен описанием текущей реализации и не заменяет код: если формулировка в документе расходится с реализацией, приоритет у кода.
...
- [Часть 3. Текущие контракты и правила](#часть-3-текущие-контракты-и-правила)
  - [Внешний контракт](#внешний-контракт)
  - [DOM-контракт](#dom-контракт)
  - [CSS-контракт](#css-контракт)
  - [Текущие инварианты](#текущие-инварианты)
...
Соответствующая реализация: [`sticky.scss`](../packages/editor/src/bundle/sticky/sticky.scss).
...
Соответствующая реализация: [`useSticky.ts`](../packages/editor/src/react-utils/useSticky.ts).
...
Соответствующая реализация: [`useTargetZIndex.ts`](../packages/editor/src/react-utils/useTargetZIndex.ts).
...
Соответствующая реализация: [`Editor.ts`](../packages/editor/src/bundle/Editor.ts).
...
## Часть 3. Текущие контракты и правила
...
- Этот DOM-маркер выступает опорной точкой для popup и menu-компонентов, которые должны отображаться поверх sticky-toolbar.
...
- `_part_left` задает геометрию левой части составного sticky-блока: для уже существующего `::before` устанавливает `right: 0`, убирает правую границу и правые радиусы.
- `_part_right` задает геометрию правой части составного sticky-блока: для уже существующего `::before` устанавливает `left: 0`, убирает левую границу и левые радиусы.
...
| Симптом | Подсистема | Что проверить в документе |
| --- | --- | --- |
...
| Toolbar липнет, но активное sticky-состояние не включается | Определение `sticky-active` | `Условия работы`, `Как работает sticky-header` / `2. Определение sticky-active`, `Текущие инварианты`, `Ограничения` |
...
| Settings остается sticky, но не становится active | Асимметрия toolbar/settings | `CSS-контракт`, `Поведенческие особенности`, `Текущие инварианты` |
