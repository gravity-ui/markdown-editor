...
function setup({
    ...
    const controller = {
        enabled,
        busy,
        start: jest.fn((request: ResourceReplacementRequest) => {
    ...
    const {schema, plugins, markupParser} = new ExtensionsManager({
        extensions(builder) {
            ...
            builder.use(ResourceReplacement, {controller, shouldTrack});
        },
    }).build();
...
describe('resourceReplacementPlugin: accepted state', () => {
    ...
    test('precomputes batches without requests and starts only after accepting the state', () => {
        ...
        expect(t.indicators()).toHaveLength(1);
    });
    ...
    test('collects one batch in final coordinates for a transaction with multiple steps', () => {
        ...
        expect(t.batches()).toEqual([
            expect.objectContaining({started: true, ranges: [{from: 8, to: 9}]}),
        ]);
        expect(t.requests[0].resources).toEqual([
    ...
    test.each(['disabled', 'untracked', 'text-only'] as const)(
        ...
        (kind) => {
            ...
            if (kind === 'untracked') t.shouldTrack.mockReturnValue(false);
            const tr =
                kind === 'text-only'
            ...
            expect(t.batches()).toEqual([]);
            expect(t.controller.start).not.toHaveBeenCalled();
            if (kind === 'disabled') expect(t.shouldTrack).not.toHaveBeenCalled();
        },
    );
...
describe('resourceReplacementPlugin: protection and history', () => {
    test('protects the inserted resource while allowing surrounding edits and existing matches', () => {
        const t = setup({initial: '![existing](/old.png)'});
        t.paste();
        const pending = t.view.state;
        t.view.dispatch(pending.tr.delete(2, 3));
        t.view.dispatch(pending.tr.setNodeAttribute(2, 'src', '/manual'));
        t.view.dispatch(pending.tr.addMark(2, 3, t.schema.marks.code.create()));
        t.view.dispatch(pending.tr.insertText('prefix', 1).setNodeAttribute(8, 'src', '/manual'));
        expect(t.view.state).toBe(pending);

        const indicator = t.indicators()[0];
        t.view.dispatch(t.view.state.tr.insertText('prefix', 1));
        expect(t.batches()[0].ranges).toEqual([{from: 8, to: 9}]);
        expect(t.indicators()[0]).toBe(indicator);
    ...
    test('service replacements bypass both history and protected-resource checks', () => {
        if (!resourceHistoryKey) throw new Error('Missing history plugin key');
        const t = setup();
        t.paste();
        t.controller.busy = true;
        const {from} = t.batches()[0].ranges[0];
...
describe('resourceReplacementPlugin: request lifecycle', () => {
    test.each([0, 1])(
        ...
        (first) => {
            ...
            expect(t.indicators()).toHaveLength(1);
            const protectedState = t.view.state;
            const {from, to} = t.batches()[0].ranges[0];
            t.view.dispatch(protectedState.tr.delete(from, to));
            expect(t.view.state).toBe(protectedState);
    ...
    test('release without a response keeps original URLs and removes protection', () => {
        const t = setup();
        t.paste();
        const {from, to} = t.batches()[0].ranges[0];
        t.requests[0].release();
    ...
    test.each(['view', 'plugin'] as const)(
        ...
        (target) => {
            ...
            const dispatch = jest.spyOn(t.view, 'dispatch');
            try {
