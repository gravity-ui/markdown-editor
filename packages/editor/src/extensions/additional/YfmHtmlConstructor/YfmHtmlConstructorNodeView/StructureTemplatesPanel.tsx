...
const StructureImportEditor: FC<{
    ...
    const handleSave = () => {
        try {
            ...
            if (parsed.length) saveTemplates(parsed);
            onClose();
        } catch (err) {
            setError(
    ...
    return (
        <div className={b('structures-import')}>
            <div className={b('structures-import-fields')}>
                <TextArea
                    controlProps={{className: stop, 'aria-label': i18n('add_template')}}
                    value={input}
                    onUpdate={(value) => {
                        setInput(value);
                        setError('');
                ...
                />
                {error && (
                    <div role="alert" className={b('structures-error')}>
                        {error}
                    </div>
                )}
            </div>
        ...
        ? [
              {
                  ...
                  render: (close) => <StructureImportEditor onClose={close} />,
              },
          ]
        : undefined;
