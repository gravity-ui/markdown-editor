...
export const LivePreview: FC<{markup: string; css: string}> = ({markup, css}) => {
    ...
    useEffect(() => {
        ...
        root.innerHTML = `<style>:host{display:block}*{box-sizing:border-box}img{max-width:100%}${HTML_CONSTRUCTOR_VARIABLES_CSS}${css}</style><div class="g-root_theme-${theme}">${markup}</div>`;
    }, [css, markup, theme]);
