...
import {cloneHtmlConstructorBlock, regenerateHtmlIds} from '../model';
import {getNextQuickStyle, normalizeHtmlConstructorQuickStyle, setThemedColor} from '../quickStyle';
...
describe('YfmHtmlConstructor utils', () => {
    ...
    it('preserves the surviving block metadata when the first wrapper is deleted', () => {
        ...
        const result = parseStructureHtml(
            '<div class="g-md-hc-block g-md-hc-block-2"><p>Edited</p></div>',
            {css: '', content: '', themeIds: []},
            blocks,
        );
    ...
    it('keeps block identity and styles when wrappers are reordered', () => {
        ...
        const result = parseStructureHtml(
            '<div class="g-md-hc-block g-md-hc-block-2"><p>Two</p></div>' +
                '<div class="g-md-hc-block g-md-hc-block-1"><p>One</p></div>',
            {css: '', content: '', themeIds: []},
            blocks,
        );
    ...
    it('gives duplicated wrappers distinct block identities', () => {
        ...
        const result = parseStructureHtml(wrapper + wrapper, {css: '', content: '', themeIds: []}, [
            block,
        ]);

        expect(result.blocks).toHaveLength(2);
        expect(result.blocks[0]).toEqual(block);
        expect(result.blocks[1]).toMatchObject({css: block.css, content: block.content});
    ...
    it('preserves SVG, comments and escaped text outside block wrappers', () => {
        ...
        const result = parseStructureHtml(content, {css: '', content: '', themeIds: []}, []);
    ...
    it('keeps valid preview styles when another stylesheet is incomplete', () => {
        ...
        const blocks = [
            ...
            {id: 'b', css: '& { padding: 12px; }', content: '', themeIds: []},
        ];
        const css = buildPreviewCss({structure, blocks, scopeSelector: '.instance'});

        expect(css).toContain(
            '.instance .g-md-hc-structure.g-md-hc-structure-1 { display: grid; }',
        );
        expect(css).toContain('.instance .g-md-hc-block.g-md-hc-block-2 { padding: 12px; }');
