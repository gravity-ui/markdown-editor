...
import {joinPreviousBlock} from 'src/commands/join';
...
export const liftFromQuote: Command = (state, dispatch) => {
    ...
    if (!$cursor || !isQuoteLinkNode($cursor.node(-1))) return false;
    return lift(state, dispatch);
};

export const joinPrevQuote = joinPreviousBlock({
    checkPrevNode: isQuoteLinkNode,
    skipNode: isQuoteLinkNode,
});
