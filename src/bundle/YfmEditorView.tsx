...
import {useYfmEditorContext} from './context';
import {EditorSettings, EditorSettingsProps} from './settings';
import {stickyCn} from './sticky';

import '../styles/styles.scss';
import './YfmEditorView.scss'; // eslint-disable-line import/order

export const cnEditorComponent = cn('editor-component');
const b = cnEditorComponent;

export type YfmEditorViewProps = ClassNameProps & {
    editor?: Editor;
    autofocus?: boolean;
...
export const YfmEditorView = React.forwardRef<HTMLDivElement, YfmEditorViewProps>((props, ref) => {
    ...
    const context = useYfmEditorContext();
    const editor = (props.editor ?? context) as EditorInt;
    if (!editor)
        throw new Error(
            '[YfmEditorView]: an instance of the editor must be passed through the props or context',
        );
    ...
    const markupSplitMode = editor.splitModeEnabled && editor.splitMode && editorType === 'markup';
...
YfmEditorView.displayName = 'YfmEditorView';
